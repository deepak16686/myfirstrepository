...
docker exec prod-splunk curl -sfk -X POST "https://localhost:8089/servicesNS/admin/splunk_httpinput/data/inputs/http" \
    -u "admin:${PROD_SPLUNK_PASSWORD:-${SPLUNK_PASSWORD}}" \
...
SPLUNK_HEC_TOKEN=$(docker exec prod-splunk curl -sfk \
    ...
    -u "admin:${PROD_SPLUNK_PASSWORD:-${SPLUNK_PASSWORD}}" 2>/dev/null | \
...
Splunk:         https://localhost:${PROD_SPLUNK_UI_PORT:-20000}    user=admin  pass=${PROD_SPLUNK_PASSWORD:-${SPLUNK_PASSWORD}}
