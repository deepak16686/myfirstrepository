...
from app.services.pipeline.stage_fixes import (
    apply_stage_fix_to_gitlab_ci,
    find_stage_fix,
    record_stage_fix,
    stage_for_job,
)
...
_GITLAB_STAGE_RANK = {
    stage: index for index, stage in enumerate(_GITLAB_STAGE_ORDER)
}
_GITLAB_JOB_ORDER = [
    "compile",
    "build_image",
...
_GITLAB_JOB_RANK = {
    job: index for index, job in enumerate(_GITLAB_JOB_ORDER)
}
...
def _job_int(value: Any, default: int = 0) -> int:
    try:
        return int(value)
...
def _ordered_gitlab_jobs(jobs_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Sort GitLab jobs in the same left-to-right order as the pipeline graph."""
    return sorted(
        jobs_list,
        key=lambda job: (
            _GITLAB_STAGE_RANK.get(str(job.get("stage") or "").lower(), 100),
            _GITLAB_JOB_RANK.get(str(job.get("name") or "").lower(), 100),
            _job_int(job.get("id")),
            str(job.get("name") or ""),
        ),
    )
...
def _normalise_gitlab_jobs(jobs_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Return the compact job contract consumed by the portal run monitor."""
    normalised = []
    for job in _ordered_gitlab_jobs(jobs_list):
        normalised.append({
            "id": _job_int(job.get("id")),
...
def _job_breakdown(jobs_list: List[Dict[str, Any]]) -> str:
    ...
    if not jobs_list:
        return ""
    parts = []
    for j in _ordered_gitlab_jobs(jobs_list):
    ...
    for check_num in range(max_checks):
        try:
            ...
            try:
                ...
                if project_id:
                    progress_store.set_jobs(
                        project_id,
                        branch,
                        _normalise_gitlab_jobs(jobs_list),
                    )
            except Exception:
                pass
...
async def monitor_pipeline_with_self_heal(
    ...
    try:
        ...
        framework = analysis.get("framework", framework)
        build_tool = (
            analysis.get("build_tool")
            or analysis.get("package_manager")
            or build_tool
        )
        print(
            "[GitLab Monitor] Repository analysis for self-heal: "
            f"language={language}, framework={framework}, "
            f"build_tool={build_tool}, "
            f"java_version={analysis.get('java_version') or analysis.get('language_version') or 'unknown'}"
        )
    except Exception as analysis_err:
        print(f"[GitLab Monitor] Repository analysis unavailable for self-heal: {str(analysis_err)[:120]}")

    self_heal_retry_notes: List[str] = []
    source_template_invalidated = False
    pending_stage_fix_record: Optional[Dict[str, Any]] = None
    max_checks = (max_wait_minutes * 60) // check_interval_seconds
    for heal_attempt in range(max_heal_attempts + 1):
        ...
        if last_status == "success":
            stage_fix_result = None
            if pending_stage_fix_record:
                pending_stage_fix_record["fixed_pipeline_id"] = last_pipeline_id
                stage_fix_result = await record_stage_fix(**pending_stage_fix_record)
                print(
                    "[GitLab Monitor] Stage-fix learning result: "
                    f"{stage_fix_result}"
                )
            ...
            if project_id:
                stage_fix_msg = ""
                if stage_fix_result and stage_fix_result.get("recorded"):
                    stage_fix_msg = "\nStage fix saved for future reuse."
                progress_store.complete(
                    project_id, branch, "success",
                    f"✅ Pipeline succeeded{attempts_msg}!{last_jobs_info}\n"
                    f"[View Pipelines]({pipelines_browser_url}){stage_fix_msg}",
                )
            return
        ...
        if source_template_id and not source_template_invalidated:
            source_template_invalidated = await pipeline_generator.mark_successful_template_failed(
                source_template_id,
                f"Direct RAG run failed with status {last_status} in pipeline #{last_pipeline_id}",
                pipeline_id=last_pipeline_id,
            )
        if project_id:
            progress_store.update(
        ...
        if error_log:
            ...
            if project_id:
                progress_store.update(
                    ...
                    "Checking learned stage fixes before LLM...",
                    attempt=heal_attempt + 1,
                )

            failed_stage = stage_for_job(
                current_gitlab_ci,
                failed_job_name,
                "unknown",
            )
            learned_fix = await find_stage_fix(
                provider="gitlab",
                stage=failed_stage,
                failed_job_name=failed_job_name or "unknown",
                error_log=error_log,
                language=language,
                framework=framework,
            ...
            if learned_fix:
                learned_gitlab_ci = apply_stage_fix_to_gitlab_ci(
                    current_gitlab_ci,
                    learned_fix,
                )
                if learned_gitlab_ci:
                    new_gitlab_ci = learned_gitlab_ci
                    fix_applied = True
                    pending_stage_fix_record = {
                        "provider": "gitlab",
                        "repo_url": repo_url,
                        "branch": branch,
                        "failed_pipeline_id": last_pipeline_id,
                        "fixed_pipeline_id": None,
                        "failed_job_name": failed_job_name or "unknown",
                        "stage": failed_stage,
                        "error_log": error_log,
                        "root_cause": (
                            learned_fix.metadata.get("root_cause")
                            or "Applied verified stage-fix template"
                        ),
                        "original_pipeline": current_gitlab_ci,
                        "fixed_pipeline": learned_gitlab_ci,
                        "dockerfile": current_dockerfile,
                        "language": language,
                        "framework": framework,
                        "build_tool": build_tool,
                        "source": "stage_fix_rag",
                    }
                    print(
                        "[GitLab Monitor] Applied learned stage fix "
                        f"{learned_fix.id} score={learned_fix.score:.2f}"
                    )
                    if project_id:
                        progress_store.update(
                            project_id,
                            branch,
                            "build_running",
                            f"Applied learned stage fix for '{failed_job_name}' "
                            "without an LLM call. Committing fix...",
                            attempt=heal_attempt + 1,
                        )
            ...
            if not fix_applied:
                if project_id:
                    progress_store.update(
                        project_id, branch, "build_running",
                        f"No verified stage fix found for '{failed_job_name}'. "
                ...
                if fix_result.success and (fix_result.gitlab_ci or fix_result.dockerfile):
                    ...
                    pending_stage_fix_record = {
                        ...
                        "error_log": error_log,
                        "root_cause": (
                            f"{fix_result.error_type}: {fix_result.explanation}"
                        )[:700],
                        "original_pipeline": current_gitlab_ci,
                        "fixed_pipeline": new_gitlab_ci,
        ...
        if not fix_applied:
            ...
            if not pending_stage_fix_record:
                pending_stage_fix_record = {
                    "provider": "gitlab",
                    "repo_url": repo_url,
                    "branch": branch,
                    ...
                    "error_log": error_log or "No failed job trace was available.",
                    "root_cause": "Static validation self-heal",
                    "original_pipeline": current_gitlab_ci,
                    "fixed_pipeline": new_gitlab_ci,
                    "dockerfile": new_dockerfile,
                    "language": language,
        ...
        if source_template_id:
            new_gitlab_ci = pipeline_generator.prepare_rag_self_heal_pipeline(
                new_gitlab_ci,
            )
        ...
        if pending_stage_fix_record:
            pending_stage_fix_record["fixed_pipeline"] = new_gitlab_ci
            pending_stage_fix_record["dockerfile"] = new_dockerfile
            pending_stage_fix_record["stage"] = stage_for_job(
                new_gitlab_ci,
                pending_stage_fix_record.get("failed_job_name"),
                pending_stage_fix_record.get("stage") or "unknown",
            )
        ...
        try:
            ...
            await ensure_images_in_nexus(f"{new_gitlab_ci}\n{new_dockerfile}")
...
async def _run_self_heal(
    ...
    try:
        ...
        if auto_commit and fix_result.get('success'):
            try:
                await ensure_images_in_nexus(
                    f"{fix_result['gitlab_ci']}\n{fix_result.get('dockerfile') or ''}"
                )
            except Exception as seed_err:
...
class GeneratePipelineResponse(BaseModel):
    ...
    template_framework: Optional[str] = None
...
async def generate_pipeline(request: GeneratePipelineRequest):
    ...
    try:
        ...
        return GeneratePipelineResponse(
            ...
            template_framework=result.get('template_framework'),
            final_errors=result.get('final_errors') or None,
        )
    except Exception as e:
        error_detail = f"{type(e).__name__}: {str(e) or repr(e)}"
        print(f"[PipelineGenerate] Error: {error_detail}")
...
async def get_best_config(
    ...
    try:
        lookup_framework = framework or ""
        if version and framework:
            normalized_version = str(version).strip().lower()
            normalized_language = language.strip().lower()
            for prefix in ("java", "jdk", "python", "py"):
                if normalized_version.startswith(prefix):
                    normalized_version = normalized_version[len(prefix):].lstrip("-_")
            if f"{normalized_language}{normalized_version}" not in lookup_framework.lower():
                lookup_framework = f"{framework}-{normalized_language}{normalized_version}"
        config = await pipeline_generator.get_best_pipeline_config(
            language=language,
            framework=lookup_framework,
            analysis={
                "language": language,
                "framework": framework or lookup_framework,
                "build_tool": (
                    "gradle" if (framework or "").lower() == "gradle"
                    else "maven" if (framework or "").lower() in {"maven", "spring-boot"}
                    else ""
                ),
                "package_manager": (
                    "gradle" if (framework or "").lower() == "gradle"
                ...
                "language_version": version or "",
                "java_version": (version or "") if language.lower() == "java" else "",
                "python_version": (version or "") if language.lower() == "python" else "",
        ...
        if config:
            return {
                ...
                "framework": framework,
                "version": version,
                "template_framework": lookup_framework,
                "config": config,
...
@router.get("/progress")
async def list_pipeline_progress():
    """List active/recent GitLab monitors for the Pipelines page."""
    return [
        {"found": True, **progress.to_dict()}
        for progress in progress_store.list()
    ]
...
async def full_workflow(
    ...
    ``chromadb-direct`` RAG hits use strict cache reuse: they run the saved
    template exactly once with status-only monitoring. They do not enter the
    LLM/self-heal loop, do not invalidate the cached template, and do not write
    another RAG record. If the cached template fails on a same-stack repo, that
    failure is surfaced as a template/runtime issue instead of being masked by
    LLM repair.
    """
    try:
        result = await pipeline_generator.generate_pipeline_files(
        ...
        if request.auto_commit:
            ...
            is_direct_artifact = result.get('model_used') == 'direct-artifact-template'
            is_rag_direct = bool(result.get('rag_hit')) or result.get('model_used') == 'chromadb-direct'
            project_id_int = int(commit_result['project_id'])
            prog = progress_store.create(
                project_id=project_id_int,
                branch=branch_name,
                max_attempts=0 if is_direct_artifact else 10,
            )
            prog.model_used = result.get('model_used', 'unknown')

            if is_direct_artifact or is_rag_direct:
                if is_rag_direct:
                    progress_message = (
                        "RAG template committed. Strict RAG-only monitor active; "
                        "no LLM/self-heal path will run."
                    )
                    monitor_mode = "rag_strict_reuse"
                else:
                    progress_message = (
                        "Direct artifact pipeline committed. Status-only monitor active; "
                        "no LLM/self-heal path is running."
                    )
                    monitor_mode = "status_only"
                progress_store.update(
                    project_id_int, branch_name, "monitoring", progress_message,
                )
                ...
                monitor_message = "Pipeline committed. Self-heal monitor active."
                progress_store.update(
                ...
                asyncio.create_task(_monitor_pipeline_safely(
                    ...
                    previous_max_pipeline_id=existing_pipeline_id,
                    source_template_id=result.get('template_id') if is_rag_direct else None,
                ))
            ...
            response["monitoring"] = {
                ...
                "project_id": project_id_int,
                "branch": branch_name,
                "rl_enabled": not (is_direct_artifact or is_rag_direct),
                "self_healing_enabled": not (is_direct_artifact or is_rag_direct),
                "monitor_mode": monitor_mode if (is_direct_artifact or is_rag_direct) else "self_heal_on_failure",
            }
...
async def commit_and_monitor(
    ...
    """Auto-seed images, commit pipeline files, then monitor the pipeline.

    This is the endpoint the chat tool calls when the user says "commit and
    run". Direct RAG hits are strict cache reuse: no LLM/self-heal loop runs.
    Other generated pipelines keep the self-healing monitor. Returns
    immediately with the commit metadata; monitoring runs in the background
    and surfaces via ``progress_store``.
    """
    try:
        branch_name = request.branch_name
        if not branch_name:
        ...
        try:
            await ensure_images_in_nexus(f"{request.gitlab_ci}\n{request.dockerfile or ''}")
        except Exception as seed_err:
        ...
        # Step 3: progress entry + monitor. Direct RAG hits are strict cache
        # reuse: no LLM/self-heal loop, no template invalidation, and no RAG
        # write-back. If a proven same-stack template fails, report that
        # failure directly so the saved template can be fixed intentionally.
        # Direct artifact mode also remains status-only because it has no
        # Dockerfile source for self-heal.
        is_direct_artifact = request.model_used == 'direct-artifact-template'
        is_rag_direct = bool(request.rag_hit) or request.model_used == 'chromadb-direct'
        project_id_int = int(commit_result['project_id'])
        prog = progress_store.create(
            project_id=project_id_int,
            branch=commit_result['branch'],
            max_attempts=0 if (is_direct_artifact or is_rag_direct) else request.max_heal_attempts,
        )
        ...
        if is_direct_artifact or is_rag_direct:
            if is_rag_direct:
                ...
                monitor_mode = "status_only"
            progress_store.update(
                project_id_int, commit_result['branch'], "monitoring", progress_message,
            )
            asyncio.create_task(monitor_pipeline_for_learning(
                repo_url=request.repo_url,
                gitlab_token=request.gitlab_token,
            ...
            asyncio.create_task(_monitor_pipeline_safely(
                ...
                previous_max_pipeline_id=existing_pipeline_id,
                source_template_id=request.template_id if is_rag_direct else None,
            ))
        ...
        return {
            ...
            "monitoring": {
                ...
                "rl_enabled": not (is_direct_artifact or is_rag_direct),
                "self_healing_enabled": not (is_direct_artifact or is_rag_direct),
                "max_heal_attempts": 0 if (is_direct_artifact or is_rag_direct) else request.max_heal_attempts,
                "monitor_mode": monitor_mode if (is_direct_artifact or is_rag_direct) else "self_heal_on_failure",
            },
        }
    except Exception as e:
