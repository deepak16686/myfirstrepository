...
from app.services.pipeline.stage_fixes import (
    apply_stage_fix_to_jenkinsfile,
    find_stage_fix,
    record_stage_fix,
)
...
PUBLIC_GITEA_BASE = "https://gitea.deepaksharma.live"
PUBLIC_JENKINS_BASE = "https://jenkins.deepaksharma.live/jenkins"
...
def _to_internal_url(url: str) -> str:
    ...
    if url.startswith(PUBLIC_GITEA_BASE):
        return settings.jenkins_git_url + url[len(PUBLIC_GITEA_BASE):]
    url = url.replace(PUBLIC_GITEA_BASE.replace("https://", ""), internal_host)
    return url


def _to_browser_url(url: str) -> str:
    """Translate Docker-internal Gitea URL to the public browser URL."""
    internal_host = settings.jenkins_git_url.replace("http://", "")
    return url.replace(internal_host, PUBLIC_GITEA_BASE.replace("https://", "")).replace(
        settings.jenkins_git_url, PUBLIC_GITEA_BASE
    )


def _jenkins_build_url(job_name: str, build_number: int | None = None) -> str:
    if build_number is None:
        return f"{PUBLIC_JENKINS_BASE}/job/{job_name}/"
    return f"{PUBLIC_JENKINS_BASE}/job/{job_name}/{build_number}/console"
    ...
    try:
        ...
        if _is_approval(message):
            ...
            if commit_result.get("success"):
                ...
                jenkins_build_url = f"{PUBLIC_JENKINS_BASE}/job/{parsed['repo']}/job/{branch}/"
...
"""
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            for coll_name in [TEMPLATES_COLLECTION, SUCCESSFUL_PIPELINES_COLLECTION]:
...
async def monitor_build_for_learning(
    ...
    for heal_attempt in range(max_heal_attempts + 1):
        if heal_attempt > 0 and project_id and branch:
            progress_store.update(project_id, branch, "build_running",
                f"Self-heal attempt {heal_attempt}/{max_heal_attempts} - watching new Jenkins build...")
        ...
        for check_num in range(max_checks):
            ...
            try:
                ...
                # Convert internal Jenkins URL to the public browser URL.
                if build_url:
                    build_url = build_url.replace(settings.jenkins_url, PUBLIC_JENKINS_BASE)

                if previous_build_number and build_number <= previous_build_number:
                    if project_id and branch:
                        progress_store.update(project_id, branch, "build_running",
                            f"Waiting for new Jenkins build after #{previous_build_number}...")
                    continue
                ...
                if project_id and branch:
                    ...
                    jenkins_link = _jenkins_build_url(job_name, build_number)
                    if build_status == "not_found":
                        progress_store.update(project_id, branch, "build_running",
                            "Waiting for Jenkins to discover the branch...")
                    elif building:
                        progress_store.update(project_id, branch, "build_running",
                            f"{attempt_prefix}Build #{build_number} running{stage_msg} | [Console]({jenkins_link})")
                    else:
                        progress_store.update(project_id, branch, "build_running",
                            f"{attempt_prefix}Build #{build_number} finished: {build_status}{stage_msg}")
        ...
        if last_build_status == "success":
            ...
            if pending_stage_fix_record:
                pending_stage_fix_record["fixed_pipeline_id"] = last_build_number
                stage_fix_result = await record_stage_fix(**pending_stage_fix_record)
                print(f"[Jenkins Monitor] Stage-fix learning result: {stage_fix_result}")
            ...
            if project_id and branch:
                ...
                stage_summary = "\n".join(stage_parts) if stage_parts else last_stage_summary
                jenkins_link = _jenkins_build_url(job_name, last_build_number)
                stage_fix_msg = ""
                if stage_fix_result and stage_fix_result.get("recorded"):
                    stage_fix_msg = "\nStage fix saved for future Jenkins reuse."
                attempts_msg = f" after {heal_attempt} fix(es)" if heal_attempt > 0 else ""
                progress_store.complete(project_id, branch, "success",
                    f"Build #{last_build_number} succeeded{attempts_msg}! \u2705\n{stage_summary}\n[View Console]({jenkins_link}){stage_fix_msg}")
            return

        if last_build_status not in ["failure", "unstable"]:
            print(f"[Jenkins Monitor] Build ended with status: {last_build_status}")
            if project_id and branch:
                progress_store.complete(project_id, branch, "failed",
                    f"Build #{last_build_number} ended with status: {last_build_status}")
            return

        if heal_attempt >= max_heal_attempts:
            print(f"[Jenkins Monitor] All {max_heal_attempts} self-heal attempts exhausted for {job_name}")
            if project_id and branch:
                progress_store.complete(project_id, branch, "failed",
                    f"Build failed after {max_heal_attempts} self-heal attempts.")
            return

        if not current_jenkinsfile:
            if project_id and branch:
                progress_store.complete(project_id, branch, "failed",
                    f"Build failed ({last_build_status}). No Jenkinsfile available for self-healing.")
            return
        if not current_dockerfile:
            current_dockerfile = jenkins_pipeline_generator._get_default_dockerfile(
                {"language": language, "framework": framework}
            )

        failed_stages = [s["name"] for s in last_stages if s["status"] == "FAILED"]
        failed_stage = failed_stages[0] if failed_stages else "unknown"
        failed_info = f" (failed at: {', '.join(failed_stages)})" if failed_stages else ""
        jenkins_link = _jenkins_build_url(job_name, last_build_number)
        print(f"[Jenkins Monitor] Build FAILED for {job_name}{failed_info} - self-heal {heal_attempt + 1}/{max_heal_attempts}")

        if project_id and branch:
            progress_store.update(project_id, branch, "build_failed",
                f"Build #{last_build_number} failed{failed_info}. [Console]({jenkins_link}) | Checking learned stage fixes...")

        error_log = await jenkins_pipeline_generator.get_build_console_log(
            job_name,
            last_build_number,
        )
        if not error_log:
            error_log = f"Jenkins build failed at stage: {failed_stage}"

        fix_applied = False
        new_jenkinsfile = current_jenkinsfile
        new_dockerfile = current_dockerfile
        learned_fix = await find_stage_fix(
            provider="jenkins",
            stage=failed_stage,
            failed_job_name=failed_stage,
            error_log=error_log,
            language=language,
            framework=framework,
        )
        if learned_fix:
            learned_jenkinsfile = apply_stage_fix_to_jenkinsfile(
                current_jenkinsfile,
                learned_fix,
            )
            if learned_jenkinsfile:
                new_jenkinsfile = learned_jenkinsfile
                fix_applied = True
                pending_stage_fix_record = {
                    "provider": "jenkins",
                    "repo_url": repo_url,
                    "branch": branch,
                    "failed_pipeline_id": last_build_number,
                    "fixed_pipeline_id": None,
                    "failed_job_name": failed_stage,
                    "stage": failed_stage,
                    "error_log": error_log,
                    "root_cause": learned_fix.metadata.get("root_cause") or "Applied verified Jenkins stage fix",
                    "original_pipeline": current_jenkinsfile,
                    "fixed_pipeline": new_jenkinsfile,
                    "dockerfile": current_dockerfile,
                    "language": language,
                    "framework": framework,
                    "source": "stage_fix_rag",
                }
                print(f"[Jenkins Monitor] Applied learned stage fix {learned_fix.id} score={learned_fix.score:.2f}")

        if not fix_applied:
            if project_id and branch:
                progress_store.update(project_id, branch, "build_failed",
                    f"No verified Jenkins stage fix found for '{failed_stage}'. LLM generating fix...")
            try:
                ...
                if fix_result.get("success"):
                    new_jenkinsfile = fix_result["jenkinsfile"]
                    new_dockerfile = fix_result["dockerfile"]
                    fix_applied = True
                    pending_stage_fix_record = {
                        "provider": "jenkins",
                        "repo_url": repo_url,
                        ...
                        "stage": failed_stage,
                        "error_log": error_log,
                        "root_cause": "Jenkins LLM self-heal",
                        "original_pipeline": current_jenkinsfile,
                        "fixed_pipeline": new_jenkinsfile,
                    ...
                    if project_id and branch:
                        progress_store.complete(project_id, branch, "failed",
                            f"Self-healing failed after {fix_result.get('attempts', 0)} attempts.")
                    return
        ...
        if not fix_applied:
            if project_id and branch:
                progress_store.complete(project_id, branch, "failed",
                    "No Jenkins self-heal fix could be applied.")
            return

        previous_build_number = last_build_number
        try:
            if project_id and branch:
                progress_store.update(project_id, branch, "build_running",
                    f"Committing Jenkins self-heal fix {heal_attempt + 1}/{max_heal_attempts}...")
            await jenkins_pipeline_generator.commit_to_repo(
                repo_url=repo_url,
                git_token=git_token,
                jenkinsfile=new_jenkinsfile,
                dockerfile=new_dockerfile,
                branch_name=branch,
            )
            current_jenkinsfile = new_jenkinsfile
            current_dockerfile = new_dockerfile
            await jenkins_pipeline_generator.trigger_scan(repo_name)
            print(f"[Jenkins Monitor] Self-heal commit #{heal_attempt + 1} pushed to {branch}")
            await asyncio.sleep(10)
        except Exception as commit_err:
            print(f"[Jenkins Monitor] Self-heal commit error: {commit_err}")
            if project_id and branch:
                progress_store.complete(project_id, branch, "failed",
