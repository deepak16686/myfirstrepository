...
from app.services.pipeline.stage_fixes import (
    apply_stage_fix_to_gitea_workflow,
    find_stage_fix,
    record_stage_fix,
    stage_for_workflow_job,
)
...
def _to_internal_url(url: str) -> str:
    ...
    if url.startswith(PUBLIC_GITEA_BASE):
        return settings.github_url + url[len(PUBLIC_GITEA_BASE):]
...
def _to_browser_url(url: str) -> str:
    ...
    return url.replace(internal_host, PUBLIC_GITEA_BASE.replace("https://", "")).replace(
        settings.github_url, PUBLIC_GITEA_BASE
    )
    ...
    try:
        ...
        if url:
            ...
            if not result.get("success") and not result.get("workflow"):
                return {
                    ...
                    "message": f"**Error generating workflow.** Could not create a valid Gitea Actions workflow for `{display_url}`.\n\nPlease check the repository URL and try again."
                }
            ...
            response_msg = (
                f"{source_msg}"
                f"Generated **Gitea Actions workflow** and **Dockerfile** for `{display_url}`\n\n"
                f"- Language: **{lang}**\n"
                f"- Framework: **{framework}**\n\n"
                f"{val_msg}"
                f"### .gitea/workflows/ci.yml\n```yaml\n{result.get('workflow', '')[:3000]}\n```\n\n"
        ...
        if _is_approval(message):
            ...
            if not pending:
                return {
                    "conversation_id": conversation_id,
                    "message": "No workflow to commit. Please provide a repository URL first to generate a Gitea Actions workflow."
                }
            ...
            if commit_result.get("success"):
                ...
                gitea_actions_url = f"{PUBLIC_GITEA_BASE}/{parsed['owner']}/{parsed['repo']}/actions"
        ...
        if _is_status_check(message):
            ...
            return {
                "conversation_id": conversation_id,
                "message": "No active workflow session. Provide a repository URL to generate a Gitea Actions workflow."
            }
        ...
        return {
            "conversation_id": conversation_id,
            "message": (
                "I can help you generate Gitea Actions workflows with a full **9-job** pipeline. Here's what I can do:\n\n"
...
async def generate_workflow(request: GenerateWorkflowRequest):
    """
    Generate Gitea Actions workflow and Dockerfile for a repository.
...
async def store_feedback(request: FeedbackRequest):
    ...
    try:
        ...
        differences = await compare_and_learn(
            ...
            branch=request.branch,
            generated_files={
                ".gitea/workflows/ci.yml": request.original_workflow,
                "Dockerfile": request.original_dockerfile
            }
        )

        workflow_diff = differences.get(".gitea/workflows/ci.yml", {})
...
async def store_template(request: StoreTemplateRequest):
    ...
    try:
        ...
        doc_content = f"""## Manual Gitea Actions Workflow Template
    ...
    for heal_attempt in range(max_heal_attempts + 1):
        ...
        if last_conclusion == "success":
            ...
            if pending_stage_fix_record:
                pending_stage_fix_record["fixed_pipeline_id"] = last_run_id
                stage_fix_result = await record_stage_fix(**pending_stage_fix_record)
                print(f"[GitHub Monitor] Stage-fix learning result: {stage_fix_result}")
        ...
        fix_applied = False
        original_workflow_for_fix = current_workflow
        if error_log:
            print(f"[GitHub Monitor] Got error log from job '{failed_job_name}' ({len(error_log)} chars)")
            if project_id:
                progress_store.update(project_id, branch, "build_failed",
                    f"Got error log from '{failed_job_name}'. Checking learned stage fixes before LLM...")

            failed_stage = stage_for_workflow_job(
                current_workflow,
                failed_job_name,
                failed_job_name or "unknown",
            )
            learned_fix = await find_stage_fix(
                provider="gitea",
                stage=failed_stage,
                failed_job_name=failed_job_name or failed_stage,
                error_log=error_log,
                language=language,
                framework=framework,
            )
            if learned_fix:
                learned_workflow = apply_stage_fix_to_gitea_workflow(
                    current_workflow,
                    learned_fix,
                )
                if learned_workflow:
                    current_workflow = learned_workflow
                    fix_applied = True
                    pending_stage_fix_record = {
                        "provider": "gitea",
                        "repo_url": repo_url,
                        "branch": branch,
                        "failed_pipeline_id": last_run_id,
                        "fixed_pipeline_id": None,
                        "failed_job_name": failed_job_name or failed_stage,
                        "stage": failed_stage,
                        "error_log": error_log,
                        "root_cause": learned_fix.metadata.get("root_cause") or "Applied verified Gitea Actions stage fix",
                        "original_pipeline": original_workflow_for_fix,
                        "fixed_pipeline": current_workflow,
                        "dockerfile": current_dockerfile,
                        "language": language,
                        "framework": framework,
                        "source": "stage_fix_rag",
                    }
                    print(f"[GitHub Monitor] Applied learned stage fix {learned_fix.id} score={learned_fix.score:.2f}")

            if not fix_applied:
                if project_id:
                    progress_store.update(project_id, branch, "build_failed",
                        f"No verified Gitea stage fix found for '{failed_job_name}'. LLM generating fix...")
                ...
                if fix_result.success and (fix_result.workflow or fix_result.dockerfile):
                    ...
                    pending_stage_fix_record = {
                        ...
                        "stage": failed_stage,
                        "error_log": error_log,
                        "root_cause": f"Gitea Actions LLM self-heal: {fix_result.explanation}"[:700],
                        "original_pipeline": original_workflow_for_fix,
                        "fixed_pipeline": current_workflow,
                        "dockerfile": current_dockerfile,
                        "language": language,
            ...
            failed_stage = failed_job_name or "unknown"
        ...
        if not fix_applied:
            ...
            if static_fix.get("success"):
                ...
                pending_stage_fix_record = {
                    ...
                    "failed_job_name": failed_job_name or failed_stage,
                    "stage": failed_stage,
                    "error_log": error_log or f"Gitea Actions run failed at job: {failed_stage}",
                    "root_cause": "Gitea Actions static validation self-heal",
                    "original_pipeline": original_workflow_for_fix,
                    "fixed_pipeline": current_workflow,
                    "dockerfile": current_dockerfile,
                    "language": language,
