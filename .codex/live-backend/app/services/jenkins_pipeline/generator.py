...
def _requested_language_version(language: str, analysis: Dict[str, Any], additional_context: str) -> str:
    context = additional_context or ""
    language_value = (language or "").lower()
    if language_value == "java":
        detected = analysis.get("java_version") or analysis.get("language_version") or ""
        requested = re.search(r"\b(?:java|jdk)\s*[-_/]?\s*(8|11|17|21|26)\b", context, re.I)
        return requested.group(1) if requested else str(detected or "")
    if language_value == "python":
        detected = analysis.get("python_version") or analysis.get("language_version") or ""
        requested = re.search(r"\b(?:python|py)\s*[-_/]?\s*(3\.(?:8|9|10|11|12|13))\b", context, re.I)
        return requested.group(1) if requested else str(detected or "")
    detected = analysis.get(f"{language_value}_version") or analysis.get("language_version") or ""
    requested = re.search(rf"\b{re.escape(language_value)}\s*[-_/]?\s*([0-9]+(?:\.[0-9]+)?)\b", context, re.I)
    return requested.group(1) if requested else str(detected or "")
...
class JenkinsPipelineGeneratorService:
    ...
    async def get_best_template_files(
        ...
        return await get_best_template_files(language, framework, version=version)
    ...
    async def get_build_console_log(
        ...
        """Get Jenkins console log text."""
        return await get_build_console_log(job_name, build_number)
    ...
    async def generate_pipeline_files(
        ...
        requested_version = _requested_language_version(language, analysis, additional_context)

        print(f"[Jenkins Pipeline] Analyzing {repo_url}: {language}/{framework} version={requested_version or 'default'}")

        # Priority 1: Check for proven successful template
        best_template = await self.get_best_template_files(language, framework, version=requested_version)
        if best_template and best_template.get("jenkinsfile"):
            ...
            jenkinsfile = self._validate_and_fix_jenkinsfile(
                _ensure_error_tolerance(self._ensure_learn_stage(best_template["jenkinsfile"])),
                None,
            )
            dockerfile = self._validate_and_fix_dockerfile(
                best_template.get("dockerfile") or self._get_default_dockerfile(analysis),
                language,
            )
            ...
            jenkinsfile = self._validate_and_fix_jenkinsfile(jenkinsfile, None)
            dockerfile = self._validate_and_fix_dockerfile(dockerfile, language)
        ...
        if use_template_only:
            ...
            jenkinsfile = self._validate_and_fix_jenkinsfile(
                self._ensure_learn_stage(jenkinsfile),
                None,
            )
            dockerfile = self._validate_and_fix_dockerfile(dockerfile, language)
...
"""
    ...
    def _parse_llm_output(self, text: str) -> Optional[Dict[str, str]]:
        ...
        if dockerfile_match:
            ...
            df = df.replace('ai-nexus:5001', 'nexus-docker.deepaksharma.live')
