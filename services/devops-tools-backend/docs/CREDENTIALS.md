...
NEXUS_PASSWORD=                # set from Vault / .env — never commit a real value
...
SONARQUBE_PASSWORD=            # set from Vault / .env — never commit a real value
...
```bash
# Use environment variables (load from Vault or your password manager)
export GITLAB_TOKEN="$(vault kv get -field=token secret/devops/gitlab)"
...
type: Opaque
stringData:
  # NEVER commit real values here — apply this Secret out-of-band
  # (kubectl create secret / SealedSecrets / External Secrets Operator).
  GITLAB_TOKEN: "<REPLACE_ME>"
  NEXUS_PASSWORD: "<REPLACE_ME>"
...
# Test Nexus registry — NEXUS_PASSWORD from env/Vault
curl -u "${NEXUS_USERNAME:-admin}:${NEXUS_PASSWORD}" \
